...
1.  **Facts on the Ground (Actions)**:
    - We record _what happened_, not the resulting state.
...
3.  **Validation**: Actions should be validated _before_ being broadcast. Optionally, the server (Firestore Security Rules or Cloud Functions) validates them again.
